// ============================================================
// Creator Campaign Tracker — Single-File React App
// ============================================================
// To run: serve this directory with any static server
...
// ============================================================
// STORAGE HELPERS — Uses window.storage (persistent key-value)
// ============================================================
...
function getCurrentStreak(campaignDays, daysData) {
  ...
  // Walk backwards from today (or end of campaign if past)
    ...
    <div className="bg-dark-card rounded-2xl p-5 mb-4">
      ...
      <div className="grid grid-cols-7 gap-2 mb-2">
        {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
          <div
            key={i}
            className="text-center text-xs text-muted font-semibold"
          >
            {d}
          </div>
        ))}
      ...
      {(() => {
        const startDow = parseDate(config.startDate).getDay(); // 0=Sun
        ...
        const dayCells = campaignDays.map((dateStr, idx) => {
          ...
          if (isPosted) {
            ...
            bgClass = "bg-dark-bg border-muted/30";
          } else if (isFuture) {
            bgClass = "bg-dark-bg border-dark-card";
          }

          const todayClass = isToday
            ? "border-accent pulse-today"
            : "";

          const textColor = isPosted
            ? "text-success"
            : isPast
            ? "text-muted"
            : "text-white";
          ...
          return (
            ...
            >
              ...
              {isPosted && (
                <span className="text-success text-[10px] leading-none mt-0.5">
                  ✓
                </span>
              )}
              {isToday && !isPosted && (
      ...
      <div className="flex items-center gap-4 mt-4 text-xs text-muted">
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded bg-success/20 border border-success inline-block" />{" "}
          Posted
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded bg-dark-bg border border-muted/30 inline-block" />{" "}
          Missed
        </span>
        <span className="flex items-center gap-1">
...
// --- Video Log ---
function VideoLog({ config, userData, campaignDays, onUpdateLink }) {
  const today = getTodayISO();
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                {/* Day number */}
                <span className="text-xs text-muted font-mono w-8 shrink-0">
                  D{idx + 1}
                </span>
                {/* Date */}
              ...
              {/* Link input - only show for posted or today */}
